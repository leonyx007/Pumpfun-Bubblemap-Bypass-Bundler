...
export function readWalletJson(filename: string = "wallets.json"): string[] {
...
export const saveNewFile = (newData: string[], fileName: string = "data.json") => {
  const folderPath = 'keys';
  const filePath = path.join(folderPath, fileName);
  ...
  try {
    ...
    console.log("data.json formated successfully.");
